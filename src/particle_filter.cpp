...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  // Resize weights vector and particles vector
  weights.resize(num_particles);
  particles.resize(num_particles);
  
  // Define random value engine
  default_random_engine gen;
  
  // Creates Gaussian distribution for x, y and theta (yaw).
...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
  ...
  for (int i = 0; i < num_particles; ++i) {
    ...
    // Soft check if yaw_rate is 0 or not.
    if (fabs(yaw_rate) > 0.0001) {
      ...
      // No update for theta required due to no yaw_rate
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  // This variable is used for normalizing weights of all particles
  double weight_normalizer = 0.0;
  
  // static value calculation outside the loop below
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    // Filter landmarks to keep only in the sensor_range of current particle. Push them to predictions vector.
